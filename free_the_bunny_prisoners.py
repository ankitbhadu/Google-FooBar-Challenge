...
def solution(num_buns,num_required):
    ...
    x=list(itertools.combinations(range(num_buns),num_required))
    y= list(itertools.combinations(range(num_buns),copies_per_key))
    s=[[] for _ in range(num_buns)]
    z=(num_required*len(x))//copies_per_key
    for _ in range(z):
        for i in y[_]:
            s[i].append(_)
    return s
